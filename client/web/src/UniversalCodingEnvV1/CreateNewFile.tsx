...
function CreateNewFile({ setIsModalOpen, codeWorkSessionApiRef, setFiles }) {
  ...
  const handleClick = async () => {
    ...
    setIsLoading(true);
    const result = await codeWorkSessionApiRef.createNewFile({
      filename,
      id: codeWorkSessionApiRef.codeWorkSession._id,
    });
    setIsLoading(false);
    if (!result.succeeded) {
      logger({
